...
| ID | Item | Repo | Status | Blocked by |
|---|---|---|---|---|
| [049](backlog/049-web-app-petunjuk-pengisian-semua-alat-ukur.md) | Web app: **petunjuk pengisian di semua alat ukur** meniru pop-up DCS (046) — WCP, OPM, Time Study, Task Inventory Tahap 1/2/3 | web-app | Selesai 2026-07-16 — komponen bersama `src/components/petunjuk-modal.tsx` (mekanik diekstrak dari `petunjuk-dcs.tsx` apa adanya: hand-rolled overlay, tanpa portal/dependency, `useId()` utk `aria-labelledby`, tutup via backdrop/X/CTA/Escape). **DCS di-refactor** memakainya tanpa ubah teks → `src/test/petunjuk-dcs.test.tsx` (7 test) tetap hijau tanpa diubah. 6 komponen konten baru co-located (`petunjuk-wcp/opm/ts/tahap1/tahap2/tahap3.tsx`) dgn konten dari kode aktual (WCP="Workplace Climate Profile", anchor OPM `opm-form.tsx`, kategori TS `ts-log-form.tsx`, kolom CalHR `@/components/calhr`). `defaultOpen`: WCP/OPM `!sudah_submit`; TS `aktif && logs.length===0`; TI-1 `!tahap1_submit && status==="TAHAP1"`; TI-2 `canEdit && status==="TAHAP2"`; TI-3 `!tahap3_submit && status==="TAHAP3"`. Header WCP/OPM/TI-1/TI-2/TI-3 → `flex justify-between`; TS tombol di samping "+ Tambah Log". Form (`*-form.tsx`) tak disentuh. Test baru `petunjuk-modal.test.tsx` (8) + 1 per komponen konten; `make test` hijau **262 test/46 file**, `npm run build` sukses. `docs-usage/ik/{wcp,opm,time-study,task-inventory}.md`, CHANGELOG.md & CLAUDE.md (Revisi Desain) diperbarui. Belum di-commit | — |
| [048](backlog/048-mcp-lengkapi-opm.md) | MCP: lengkapi domain OPM (15 tool) + `partisipan_saya` | mcp | Selesai 2026-07-15 — 15 tool OPM baru (`daftar/buat/cari/detail/perbarui_opm_sesi`, `opm_buka_sesi`, `opm_tutup_sesi`, `opm_daftar_task`, `opm_daftar_responden`, `opm_detail_responden`, `opm_submit_jawaban` [PUT `.../jawaban`→POST `.../jawaban/submit`], `opm_daftar_jawaban`, `opm_analisis`, `opm_hasil`, `opm_kuesioner_saya`) + `partisipan_saya` — total tool **140→158** (`grep -c '@mcp.tool'`). Semua path/verb diverifikasi langsung ke source backend v0.37.0 (`api/router.py` prefix `/opm/sesi`, bukan dari `openapi.json` yang stale di 0.36.0). OPM tetap ber-`sesi_id` + `periode`/`min_responden`/`max_responden` (tidak ikut `cabang` TI). 16 test happy-path baru + assert urutan PUT→POST `opm_submit_jawaban`. `make test` hijau **106 test**. Belum di-commit | — |
| [047](backlog/047-mcp-dcs-wcp-reset-instrumen.md) | MCP: `dcs_reset_instrumen`/`wcp_reset_instrumen` — expose endpoint reset admin 043 | mcp | Selesai 2026-07-15 — 2 tool destruktif baru → `POST /api/v1/{dcs,wcp}/instrumen/reset` (path diverifikasi ke `dcs_instrumen.py`/`wcp_instrumen.py` backend, `operation_id` `{dcs,wcp}_instrumen_reset`, `response_model=…InstrumenRead`). Docstring eksplisit menyatakan destruktif (hapus SELURUH responden+jawaban, status→OPEN, sah dari status apa pun); `*_buka_ulang_instrumen` **tidak diubah** (tetap non-destruktif `CLOSED→OPEN`). 2 test + entri daftar registrasi. CLAUDE.md (mcp) invariant diperluas membedakan `buka_ulang` vs `reset`. `make test` hijau. Belum di-commit | 043 |
| [045](backlog/045-mcp-selaraskan-buat-ti-sesi-cabang.md) | MCP: `buat_ti_sesi` (`periode`→`cabang`) & docstring `ti_tambah_responden_banyak` | mcp | Selesai 2026-07-15 — `buat_ti_sesi`: param `periode: str` → `cabang: Literal["Bandung","Semarang"]` (wajib), body kirim `cabang`. **Penyimpangan dari rencana (dalam eskalasi yang diizinkan file):** param `unit` **dihapus** — verifikasi kode menegaskan `unit` sudah dibuang dari `TiSesiCreate` backend sejak `[2026-06-25]` + `extra="forbid"`, jadi memasoknya selalu 422 (dead+menyesatkan). Docstring `ti_tambah_responden_banyak` hapus `kapasitas_penuh` (cap TI dicabut di 037). **Temuan tambahan (tak disebut file backlog):** `perbarui_ti_sesi` juga rusak oleh 037 — `TiSesiUpdate` kini hanya `cabang`/`koordinator_id`/`catatan` (`extra="forbid"`), jadi param `periode`/`min_responden`/`max_responden` diganti `cabang` + dibuang; docstring `daftar/detail/cari_ti_sesi` yang masih menyebut `unit`/`periode` ikut dirapikan. Test `test_buat_ti_sesi` diperbarui + negative-path `cabang` invalid + `test_perbarui_ti_sesi_cabang`. Versi `__init__.py` dikoreksi dari stale `0.12.0`→`0.14.0` (tertinggal sejak 0.13.x). `make test` hijau **107 test**. Belum di-commit | 037 |
